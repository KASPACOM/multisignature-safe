/**
 * Универсальный компонент для отображения информации о контракте с использованием TypeChain
 */
...
export const ContractInfo: React.FC<ContractInfoProps> = ({ contractAddress, safeOnChain }) => {
  ...
  const loadContractInfo = async () => {
    ...
    try {
      console.log('🔍 Загружаем информацию о контракте:', contractAddress)
      
      const info = safeOnChain.getContractInfo(contractAddress)
      if (info) {
        setContractData(info)
        console.log('✅ Информация о контракте загружена:', info)
      } else {
        setError('Контракт не найден в реестре')
      }
    } catch (err: any) {
    ...
    return (
      <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg">
        <div className="flex items-center space-x-2">
          ...
          <span className="text-blue-800 text-sm">Загружаем информацию о контракте...</span>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="p-4 bg-gray-50 border border-gray-200 rounded-lg">
      <div className="flex items-center space-x-2 mb-3">
        ...
        <h4 className="text-gray-800 font-medium">📋 Контракт подключен</h4>
      </div>
      ...
      <div className="mt-3 text-sm text-gray-600">
        Функций доступно для Safe пропозалов: 
