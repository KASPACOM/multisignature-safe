...
export const FunctionSelector: React.FC<FunctionSelectorProps> = ({
  ...
  const [filterPayable, setFilterPayable] = useState<'all' | 'payable' | 'nonpayable'>('all')
  ...
  const filteredFunctions = functions.filter(func => {
    ...
    const matchesPayable = filterPayable === 'all' ||
                    ...
                    <div className="space-y-1">
                      {func.inputs.map((input, index) => (
                        <div key={index} className="text-xs text-gray-600 flex">
                          ...
                          <span className="font-medium">{input.name || `param${index}`}</span>
                        </div>
                      ))}
                    </div>
                  </div>
