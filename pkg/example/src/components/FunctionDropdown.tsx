...
export const FunctionDropdown: React.FC<FunctionDropdownProps> = ({
  ...
  useEffect(() => {
    if (contractAddress) {
      ...
      contractFunctions = contractFunctions.filter(
      ...
      console.log(
        `State-changing functions loaded for Safe proposal:`,
        contractFunctions.length
      );
      console.log(
        `   - Total functions in ABI: ${
          contractRegistry.getContractFunctions(contractAddress).length
        }`
      );
