...
import { getCurrentNetworkConfig } from "./safe-common";
...
export class SafeOffChain {
  ...
  private async initializeApiKit() {
    try {
      ...
      this.apiKit = new SafeApiKit({
        txServiceUrl: this.networkConfig.stsUrl + "/api",
  ...
  async proposeUniversalResult(
    ...
    const signature = universalResult.safeTransaction.signatures?.get(
  ...
  async getUserProposals(filter: UserProposalsFilter): Promise<UserProposal[]> {
    ...
    const userSafes = filter.safeAddress
      ? [filter.safeAddress]
      : await this.getUserSafes(filter.userAddress);
  ...
  private filterAndSortProposals(
    ...
    if (filter.requiresUserSignature) {
      filtered = filtered.filter((proposal) => {
        ...
        const userHasSigned =
          proposal.confirmations?.some(
            (conf: SafeMultisigConfirmationResponse) =>
              conf.owner.toLowerCase() === filter.userAddress.toLowerCase()
          ) || false;
  ...
  private async getSTSProposalsOnly(
    safeAddress: string
