...
export const getSafeConfig = async (
  ...
  if (options.safeAddress) {
    ...
    throw new Error("Either safeAddress or predictedSafe must be specified");
  }
};
...
export const WALLET_TRANSITIONS: StateTransition[] = [
  ...
  {
    from: WalletState.Disconnected,
    to: WalletState.Connecting,
    trigger: "PROVIDER_DETECTED",
  },
  {
    from: WalletState.Disconnected,
    to: WalletState.Connecting,
