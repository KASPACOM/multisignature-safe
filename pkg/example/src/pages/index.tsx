...
const SafeMultisigApp: React.FC = () => {
  ...
  const [selectedContract, setSelectedContract] = useState<ContractABI | null>(
    null
  );
  const [selectedFunction, setSelectedFunction] =
    useState<ParsedFunction | null>(null);
  const [structuredFormData, setStructuredFormData] =
    useState<FunctionFormData>({
      parameters: {},
      ethValue: "0",
    });
  ...
  const [universalResult, setUniversalResult] =
    useState<UniversalOperationResult | null>(null);
      ...
      if (currentSafeAddress && currentOwners && currentThreshold) {
        ...
        setTimeout(async () => {
          try {
            ...
            showError(
              "Safe disconnected due to connection change. Please reconnect."
            );
          }
        }, 100);
      }
  ...
  const getHighestNonce = async (
    safeOnChainInstance?: SafeOnChain
  ): Promise<number> => {
    ...
    if (safeOffChain?.isSTSAvailable()) {
      try {
        const stsNextNonce = await safeOffChain.getNextNonce(
          safeInstance.currentSafeAddress
        );
        // Use the highest nonce
        highestNonce = Math.max(onchainSafeData.nonce, stsNextNonce);
        console.log(
          `Highest nonce: ${highestNonce} (onchain: ${onchainSafeData.nonce}, offchain current: ${stsNextNonce})`
        );
      } catch (error) {
        console.warn(
          "Could not get nonce from STS, using onchain nonce:",
          error
        );
      }
    } else {
  ...
  const handleNavigateToSafeCreation = async (
    safeAddress: string,
    owners: string[],
    threshold: number
  ) => {
    console.log("Navigation to Safe creation with data:", {
      safeAddress,
      owners,
      threshold,
    });
  ...
  const handleConnectToSafe = async (formData: SafeConnectionFormData) => {
    ...
    try {
      ...
      showError(
        error instanceof Error ? error.message : "Safe connection error"
      );
    } finally {
  ...
  const handlePredictSafeAddress = async (formData: SafeCreationForm) => {
    ...
    try {
      ...
      showError(
        error instanceof Error ? error.message : "Address prediction error"
      );
    }
  ...
  const handleCreateUniversalHash = async () => {
    ...
    try {
      ...
      const paramTypes =
        universalForm.functionSignature
          .split("(")[1]
          ?.split(")")[0]
          ?.split(",")
          ?.map((p) => p.trim())
      ...
      const paramValues = universalForm.functionParams.slice(
        0,
        paramTypes.length
      );
      ...
      if (
        ...
        try {
          ...
          console.error(
            "ETH value parsing error (manual mode):",
            universalForm.ethValue,
            parseError
          );
          throw new Error(
            `Invalid ETH value format: ${universalForm.ethValue}`
          );
        }
      }
      ...
      showSuccess(`Transaction hash created! 
        ...
        Next step: Click "Sign EIP-712 Hash" to create signature and send to STS.`);
    } catch (error: any) {
  ...
  const sendSignatureToSTS = async (
    ...
    if (!safeOffChain || !safeInfo) {
      console.log("SafeOffChain or SafeInfo unavailable");
      return;
    }
    ...
    try {
      ...
      console.log(
        'Transaction already exists in STS. Redirecting to "My Proposals" section'
      );

      setTimeout(() => {
        console.log(
          'Switching to "My Proposals" section - transaction already exists'
        );
  ...
  const handleSignTransactionHash = async () => {
    if (!universalResult || !network || !safeOnChain || !safeInfo) {
      showError(
        "No hash to sign, wallet not connected or Safe Manager unavailable"
      );
      return;
    }
    ...
    try {
      console.log(
        "Signing transaction through Protocol Kit (EIP-712):",
      ...
      console.log(
        "Signatures in signed transaction:",
        signedSafeTransaction.signatures.size
      );
      ...
      try {
        ...
        console.warn("Failed to send EIP-712 signature to STS:", stsError);
  ...
  const updateFunctionParam = (index: number, value: string) => {
    setUniversalForm((prev) => ({
      ...prev,
      functionParams: prev.functionParams.map((param, i) =>
        i === index ? value : param
      ),
    }));
  };
                    ...
                    <div className="flex flex-wrap gap-2">
                      {getSupportedNetworks().map((network) => (
                        <div
                          ...
                          style={{
                            backgroundColor:
                              NETWORK_COLORS[network.chainId] + "20",
                          ...
                          <div className="mt-6 p-4 bg-green-50 rounded-lg">
                              ...
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                  ...
                                  <div className="mt-1 p-2 bg-white border rounded font-mono text-xs">
                                    {formatAddress(
                                      universalResult.transactionDetails.to
                                    )}
                                  </div>
                                </div>
                                ...
                                <div>
                                  ...
                                  <div className="mt-1 p-2 bg-white border rounded">
                                    {universalResult.transactionDetails.value}{" "}
                                    ETH
                                  </div>
                                </div>
                            ...
                            <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                              <p className="text-blue-800 text-sm">
                                <strong>Next steps:</strong> Click "Sign
                                Transaction" for automatic signing through your
                                wallet, or copy hash for manual signing.
                              </p>
                            </div>
                          </div>
                        )}
                        ...
                        <div className="space-y-6 mb-6">
                          ...
                          <div>
                            ...
                            <div className="space-y-3">
                              {universalForm.functionParams.map(
                                (param, index) => (
                                  <div
                                    key={index}
                                  ...
                                  >
                                    ...
                                    {universalForm.functionParams.length >
                                      1 && (
                                      <button
                                        type="button"
                                        onClick={() =>
                                          removeFunctionParam(index)
                                        }
                            ...
                            <div className="mt-3 p-3 bg-gray-50 rounded-lg text-sm text-gray-600">
                              ...
                              <ul className="space-y-1 text-xs">
                                ...
                                <li>
                                  <strong>uint256:</strong> 1000 (or 100.5 for
                            ...
                            <div className="space-y-3 text-sm">
                              ...
                              <div className="mt-4 flex flex-wrap gap-3">
                                <button
                                  onClick={handleSignTransactionHash}
                                  disabled={
                                    loading.signHash || signatureResult !== null
                                  }
                              ...
                              <div className="mt-4 p-3 bg-blue-50 border-l-4 border-blue-400">
                                <p className="text-blue-800 text-sm">
                                  <strong>Next steps:</strong> Click "Sign
                                  Transaction" for automatic signing through
                                  your wallet, or copy hash for manual signing.
                                </p>
                              </div>
                            </div>
                          </div>
                            ...
                            <div className="space-y-3 text-sm">
                              ...
                              <div className="mt-4 p-3 bg-green-50 border-l-4 border-green-400">
                                <p className="text-green-800 text-sm">
                                  <strong>Success!</strong> Transaction signed
                                  and sent to STS. Check "User Proposals"
